...
- ✅ **Live Sales Chat** (`components/SalesChat.jsx` + `.css`) — Lightweight "Ask us anything" bubble mounted globally in `App.jsx`, but auto-scoped to facility-decision routes (`/register/*` and `/doctors/:id`). Collapsed = pill with sparkle icon, notification dot and attention pulse ring; open = 360×500 glass panel with agent header (Priya Menon · Facility partnerships · Online · ~5 min SLA), welcome message, 4 canned-intent quick replies (verification / demo / pricing / SMS), a typing indicator animation, a composer, and an email fallback. Intent matching (regex over keywords) drives contextual replies from `CANNED`. Esc closes, toggle collapses to a small × when open.
...
### Follow-up enhancements (Jan 2026, fifth pass)
- ✅ **Live Sales Chat** (`components/SalesChat.jsx` + `.css`) — Lightweight "Ask us anything" bubble mounted globally in `App.jsx`, but auto-scoped to facility-decision routes (`/register/*` and `/doctors/:id`). Collapsed = pill with sparkle icon, notification dot and attention pulse ring; open = 360×500 glass panel with agent header (Priya Menon · Facility partnerships · Online · ~5 min SLA), welcome message, 4 canned-intent quick replies (verification / demo / pricing / SMS), a typing indicator animation, a composer, and an email fallback. Intent matching (regex over keywords) drives contextual replies from `CANNED`. Esc closes, toggle collapses to a small × when open.
